...
def pmx_cross(p1, p2, start, end):
    o = [None for _ in range(len(p1))]

    o[start:end + 1] = p1[start:end+1]
    ...
    for i in range(start, end + 1):
        ...
        while find_value:
            index_2 = p2.index(value_2)
            value_1 = p1[index_2]

            index_2_new = p2.index(value_1)
            if index_2_new >= start and index_2_new <= end:
                value_2 = value_1
            else:
                o[index_2_new] = p2[i]
                find_value = False
...
def pmx_crossover(p1, p2):
    point1 = random.randrange(0, len(p1))
    point2 = random.randrange(0, len(p1))
    start = min(point1, point2)
    end = max(point1, point2)
    ...
    o1 = pmx_cross(p1, p2, start, end)
    o2 = pmx_cross(p2, p1, start, end)
...
if __name__ == '__main__':
    ...
    xover = functools.partial(crossover, cross=pmx_crossover, cx_prob=CX_PROB)
